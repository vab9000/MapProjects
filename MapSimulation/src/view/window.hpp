...
class window {
    sf::RenderWindow window_ = sf::RenderWindow(sf::VideoMode(sf::Vector2u(800, 600)), "Map Simulation");
...
public:
    explicit window(simulation *simulation, drawing *drawer,
                void (simulation::*event_func)(const sf::Event &)): drawer_(drawer), simulation_(simulation), event_func_(event_func) {
        ...
        while (window_.isOpen()) {
            while (const auto event = window_.pollEvent()) {
                ...
                if (const auto &resize_data = event->getIf<sf::Event::Resized>()) {
                    sf::FloatRect view(sf::Vector2f(0, 0), sf::Vector2f(resize_data->size.x, resize_data->size.y));
