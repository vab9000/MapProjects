...
void simulation::reload_bitmap_province(const province &reload_province) {
    ...
    for (auto i = 0; i < reload_province.get_num_outline(); ++i) {
        ...
        if (data_.selected_province != nullptr && reload_province.get_base_color() == data_.selected_province->
            get_base_color()) {
        ...
        if (!updated_provinces.contains(other_province)) {
            ...
            for (unsigned int j = 0; j < other_province->get_num_outline(); ++j) {
                ...
                const auto other_index = (other_pixel[0] + other_pixel[1] * map_image_.get_width()) * 4;
    ...
    drawer_.update_map_texture(bytes_.data() + map_image_.get_width() * reload_province.get_bounds()[1] * 4,
                               sf::Vector2u(map_image_.get_width(),
                                            reload_province.get_bounds()[3] - reload_province.get_bounds()[1] + 1),
...
void simulation::start_processing() {
    ...
    bytes_ = std::vector<unsigned char>(map_image_.get_width() * map_image_.get_height() * 4);
