...
void bitmap::reload_bitmap_province(const province &reload_province, const data &data, drawing &drawer) {
    std::vector<int> x_indices(reload_province.bounds()[2] - reload_province.bounds()[0] + 1);
    std::iota(x_indices.begin(), x_indices.end(), reload_province.bounds()[0]);

    std::vector<int> y_indices(reload_province.bounds()[3] - reload_province.bounds()[1] + 1);
    std::iota(y_indices.begin(), y_indices.end(), reload_province.bounds()[1]);

#ifdef __cpp_lib_execution
    std::for_each(std::execution::par_unseq, x_indices.begin(), x_indices.end(), [&](const int i) {
        std::for_each(std::execution::par_unseq, y_indices.begin(), y_indices.end(), [&](const int j) {
            const auto index = (i + j * map_image_->width()) * 4;
                ...
                set_pixel(bytes_, index, static_cast<unsigned char>(color),
                          static_cast<unsigned char>(color >> 8), static_cast<unsigned char>(color >> 16));
        });
    });
#else
    std::ranges::for_each(x_indices, [&](const int i) {
        std::ranges::for_each(y_indices [&](const int j) {
            const auto index = (i + j * map_image_.width()) * 4;
...
void bitmap::reload_bitmap(const data &data, drawing &drawer) {
    std::vector<int> x_indices(map_image_->width());
    std::iota(x_indices.begin(), x_indices.end(), 0);

    std::vector<int> y_indices(map_image_->height());
