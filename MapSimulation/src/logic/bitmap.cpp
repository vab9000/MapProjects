...
void bitmap::init_bitmap(image *image, const data &data, drawing &drawer) {
    ...
    bytes_ = std::vector<unsigned char>(map_image_->get_width() * map_image_->get_height() * 4, 0);
...
void bitmap::reload_bitmap_province(const province &reload_province, const data &data, drawing &drawer) {
    ...
    for (auto i = 0; i < reload_province.get_num_outline(); ++i) {
        ...
        if (data.selected_province != nullptr && reload_province.get_base_color() == data.selected_province->
            get_base_color()) {
    ...
    drawer.update_map_texture(bytes_.data() + map_image_->get_width() * reload_province.get_bounds()[1] * 4,
    ...
    std::for_each(
        ...
        [&](const province &province) {
            ...
            std::for_each(std::execution::par_unseq, indices.begin(), indices.end(), [&](const int i) {
                ...
                const auto color = province.color;
                const auto index = (pixel[0] + pixel[1] * map_image_->get_width()) * 4;
                if (data.selected_province != nullptr && color == data.selected_province->color) {
                    set_pixel(bytes_, index, 255, 255, 255);
                } else {
                    if (other_province != nullptr && other_province->color != color) {
                        set_pixel(bytes_, index, 0, 0, 0);
