...
namespace {
    auto set_pixel(std::vector<uint8_t> &bytes, const size_t index, const std::array<uint8_t, 3> rgb) -> void {
        bytes[index] = rgb[0];
        bytes[index + 1] = rgb[1];
        bytes[index + 2] = rgb[2];
        bytes[index + 3] = 255;
...
namespace processing {
    ...
    bitmap::bitmap(image &image, const mechanics::data &data, drawing &drawer) : map_image_(image),
        bytes_(4ULL * image.width() * image.height(), 0) { reload_bitmap(data, drawer); }
    ...
    auto bitmap::reload_bitmap_province(const mechanics::province &reload_province, const mechanics::data &data,
        ...
        std::for_each(std::execution::par_unseq, x_indices.begin(), x_indices.end(), [&](const uint_fast32_t i) {
            std::for_each(std::execution::par_unseq, y_indices.begin(), y_indices.end(),
                          [&](const uint_fast32_t j) {
                              const auto index = (i + j * map_image_->get().width()) * 4;
                              const auto color = data.provinces().at(map_image_->get().color(i, j)).
                                      color();
                              set_pixel(bytes_, index, {
        ...
        drawer.update_map_texture(bytes_.data() + 4ULL * map_image_->get().width() * reload_province.bounds()[1],
                                  sf::Vector2u(map_image_->get().width(),
    ...
    auto bitmap::reload_bitmap(const mechanics::data &data, drawing &drawer) -> void {
        const auto x_indices = std::views::iota(0u, map_image_->get().width());
        const auto y_indices = std::views::iota(0u, map_image_->get().height());

        std::for_each(std::execution::par_unseq, x_indices.begin(), x_indices.end(), [&](const uint_fast32_t i) {
            std::for_each(std::execution::par_unseq, y_indices.begin(), y_indices.end(),
                          [&](const uint_fast32_t j) {
                              const auto index = (i + j * map_image_->get().width()) * 4;
                              const auto color = data.provinces().at(map_image_->get().color(i, j)).color();
                              set_pixel(bytes_, index, {
                                            static_cast<uint8_t>(color),
                                            static_cast<uint8_t>(color >> 8),
                                            static_cast<uint8_t>(color >> 16)
