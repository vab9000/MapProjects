...
enum class sea_tile_t :unsigned int {
    coast = to_integer_color(25, 255, 255), sea = to_integer_color(100, 200, 255),
...
inline auto generate_neighbors(const image &base_map,
    ...
    types) -> std::vector<std::unordered_set<int>> {
    ...
    while (!neighbors.empty()) {
        ...
        while (!queue.empty()) {
            ...
            for (const auto &tile_neighbors = neighbors.at(current_tile); const auto &neighbor_id : tile_neighbors) {
                if (cluster.insert(neighbor_id).second) { queue.push(neighbor_id); }
            }
...
inline auto gen_types(const image &map_image, const image &base_image) -> std::unordered_map<unsigned int, sea_tile_t> {
    ...
    for (auto i = 0; i < map_image.width(); i++) {
        for (auto j = 0; j < map_image.height(); j++) {
            const auto color = static_cast<unsigned int>(map_image(i, j));
            if (is_lake(base_image(i, j))) { types[color] = sea_tile_t::lake; }
            ...
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    if (is_land(base_image(nx, ny))) { types[color] = sea_tile_t::coast; }
                }
            }
    ...
    for (auto i = 0; const auto &cluster : neighbors) {
    ...
    for (const auto &[color, coords] : pixels) {
        if (types.contains(color)) { continue; }
        const auto average_y = static_cast<int>(std::accumulate(coords.begin(), coords.end(), 0,
...
inline auto fix_sea_tile_file() -> void {
